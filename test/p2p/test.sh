...
bash test/p2p/client.sh "$NETWORK_NAME" basic "test/p2p/basic/test.sh $N"

# test atomic broadcast:
# start client container and test sending a tx to each node
bash test/p2p/client.sh "$NETWORK_NAME" ab "test/p2p/atomic_broadcast/test.sh $N"

# test fast sync (from current state of network):
# for each node, kill it and readd via fast sync
bash test/p2p/fast_sync/test.sh "$NETWORK_NAME" "$N" "$PROXY_APP"

# test killing all peers 3 times
bash test/p2p/kill_all/test.sh "$NETWORK_NAME" "$N" 3

# test pex
bash test/p2p/pex/test.sh "$NETWORK_NAME" "$N" "$PROXY_APP"
